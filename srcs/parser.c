...
static int	validate_config(t_config *config)
{
	...
	if (!config->ip && !config->file && config->hosts_count == 0 && config->ips_count == 0)
	{
		fprintf(stderr, "Error: No targets specified. Use --ip, --file, or hostnames\n");
		return (-1);
	}
	
	// Check for conflicting options
	int target_methods = 0;
	if (config->ip) target_methods++;
	if (config->file) target_methods++;
	if (config->hosts_count > 0) target_methods++;
	
	if (target_methods > 1)
	{
		fprintf(stderr, "Error: Cannot mix --ip, --file, and hostname arguments\n");
		return (-1);
	}
	...
	while (token)
	{
		...
		if (dash)
		{
			*dash = '\0';
			...
			if (!is_valid_port(token) || !is_valid_port(dash + 1) || start > end)
			{
				free(copy);
				free(list);
				free(config->ports);
	...
	while (i < argc)
	{
		...
		if (is_valid_argument(argv[i]))
		{
			...
			else if (strcmp(argv[i], "--file") == 0)
			{
				...
				{
					fprintf(stderr, "Error: Failed to parse hosts from file '%s'\n", argv[i + 1]);
					return (-1);
				}
				i += 2;
			}
