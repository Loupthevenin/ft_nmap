...
static int	validate_config(t_config *config)
{
	...
	if (config->file && (!config->ips_list || config->ips_count == 0))
	{
		fprintf(stderr, "Error: No valid IPs found in file\n");
		return (-1);
	}
	if (config->speedup <= 0)
	{
...
// TODO - Penser a parser les elements dans le fichier si --file est utilise
// Format du fichier : une IP par ligne
// TODO - Penser a parser les ports dans le fichier si --ports est utilise
// Format du fichier : ports sous forme de plage (1-10) ou liste (1,2,3)
char **parse_ips_from_file(const char *filename, int *count)
{
    ...
    while (fgets(line, sizeof(line), file))
    {
        ...
        if (is_valid_ip(line))
        {
            char **tmp = realloc(ips, sizeof(char *) * (i + 1));
            if (!tmp)
            {
                fclose(file);
                // libérer la mémoire déjà allouée
                for (int j = 0; j < i; j++) free(ips[j]);
                free(ips);
                return NULL;
            }
            ips = tmp;
...
static int	parse_ports(t_config *config, const char *ports_str)
{
	...
	if (!config->ports)
		return (0);
	int *list = malloc(sizeof(int) * 2000); // taille max
	if (!list)
	{
		free(config->ports);
		return (0);
...
static int	add_scan_type(t_config *config, const char *scan)
{
	...
	{
		fprintf(stderr,
				"Error: Invalid scan type '%s'. Must be one of: SYN,"
...
int	parse_args(t_config *config, int argc, char **argv)
{
	...
	while (i < argc)
	{
		...
		else if (strcmp(argv[i], "--file") == 0)
		{
			...
			config->ips_list = parse_ips_from_file(config->file,
					&config->ips_count);
			if (!config->ips_list || config->ips_count == 0)
			{
				fprintf(stderr, "Error: Failed to parse IPs from file '%s'\n",
					config->file);
				return (-1);
			}
			i += 2;
